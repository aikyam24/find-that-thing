...
function effectiveDate(page) {
  return page.lastVisitedAt ?? page.capturedAt ?? page.savedAt ?? page.createdAt ?? 0;
}
...
export function searchLocal(pages, query, { includeUnmatched = false, limit = 30 } = {}) {
  ...
  return pages.map(page => {
    ...
    .sort((a, b) => b.localScore - a.localScore || effectiveDate(b.page) - effectiveDate(a.page) || a.page.id.localeCompare(b.page.id))
    .slice(0, limit);
}
...
export function toResult(candidate, judgement = null) {
  ...
  return {
    ...
    capturedAt: page.capturedAt, lastVisitedAt: page.lastVisitedAt, displayedAt: effectiveDate(page), truncated: page.truncated,
